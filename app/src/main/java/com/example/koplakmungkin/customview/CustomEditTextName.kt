...
class CustomEditTextName : AppCompatEditText, View.OnTouchListener {
    ...
    private fun showClearButton() {
        setButtonDrawables(endOfTheText = nameButtonImage)
    }
    ...
    override fun onTouch(v: View?, event: MotionEvent): Boolean {
        if (compoundDrawables[2] != null) {
            ...
            if (layoutDirection == View.LAYOUT_DIRECTION_RTL) {
                clearButtonEnd = (nameButtonImage.intrinsicWidth + paddingStart).toFloat()
                when {
                    event.x < clearButtonEnd -> isClearButtonClicked = true
                }
            } else {
                clearButtonStart = (width - paddingEnd - nameButtonImage.intrinsicWidth).toFloat()
                when {
            ...
            if (isClearButtonClicked) {
                when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        nameButtonImage = ContextCompat.getDrawable(
                            context,
